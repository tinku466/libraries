...
extension UIView {
    ...
    /// Set Gradient color
    /// - Parameter colorOne: first coloe
    /// - Parameter colorTwo: second color
...
// MARK:- BORDER
extension UIView {
    /// View side for border
    enum ViewSide {
        case left, right, top, bottom
    }
    
    /// Example use: myView.addBorder(toSide: .Left, withColor: UIColor.redColor().CGColor, andThickness: 1.0, withMargin: 16.0)
    func addBorder(toSide side: ViewSide, withColor color: CGColor, andThickness thickness: CGFloat, withMargin margin: CGFloat = 0.0) {
        let border = CALayer()
        border.backgroundColor = color
        switch side {
        case .left: border.frame = CGRect(x: frame.minX, y: frame.minY, width: thickness, height: frame.height); break
        case .right: border.frame = CGRect(x: frame.maxX, y: frame.minY, width: thickness, height: frame.height); break
        case .top: border.frame = CGRect(x: margin, y: frame.minY, width: frame.width - (margin * 2), height: thickness); break
        case .bottom: border.frame = CGRect(x: margin, y: frame.maxY, width: frame.width - (margin * 2), height: thickness); break
        }
